...
from Components.azureSQL import upload_data_sql, fetch_sql_data
...
class TickerData:
    ...
    def fetch_stock_data(self, workers=20):
        ...
        self.stock_data = fetch_sql_data('us_equities')
        date = pd.to_datetime(self.stock_data['date'], utc=True).dt.tz_convert('America/New_York')
        self.stock_data['date'] = date.dt.normalize()

        unique_tickers = pd.Series(self.stock_data["ticker"].dropna().unique())
        sampled_tickers = unique_tickers.sample(n=2000, random_state=42).tolist()

        # 2. Filter as before
        self.stock_data = self.stock_data[self.stock_data["ticker"].isin(sampled_tickers)].dropna()
    ...
    def preprocess_data(self):
        self.dataset_ex_df = (
            self.stock_data
            .rename(columns={"ticker": "Ticker","open": "Open", "high": "High", "low": "Low", "close": "Close", "volume": "Volume"})
        )
        ...
        self.dataset_ex_df = self.dataset_ex_df.set_index('date')
    ...
    def stochastic_rsi(series, rsi_period=14, stoch_period=14):
        ...
        rsi = 100 - (100 / (1 + rs))
        return (rsi - rsi.rolling(stoch_period).min()) / (
    ...
    def dmd_prediction(series, modes=2):
        """
        Apply Dynamic Mode Decomposition to predict future values.

        Parameters:
            series: Time series data (Close prices)
            modes: Number of DMD modes to use

        Returns:
            Series with DMD predictions
        """
        try:
            # Check if data contains NaN or infinity values
            if series.isna().any() or np.isinf(series).any():
                # Option 1: Fill NaN/inf values with interpolation
                series = series.interpolate(method='linear').fillna(method='bfill').fillna(method='ffill')

                # If we still have NaN/inf values after filling, return zeros or original series
                if series.isna().any() or np.isinf(series).any():
                    return pd.Series(np.zeros(len(series)), index=series.index)

            # Convert to 2D array for DMD
            ...
            # Reconstruct and return the series
            reconstruction = dmd.reconstructed_data.real
            return pd.Series(reconstruction.flatten(), index=series.index)

        except Exception as e:
            # Fallback: return the original series or zeros
            print(f"DMD calculation failed: {e}")
            return pd.Series(np.zeros(len(series)), index=series.index)

    @staticmethod
    def ssa_trend(series, window_size=20):
        """Apply Singular Spectrum Analysis for trend extraction.
        Adapts window size for shorter time series.
        """
        # Ensure window size is valid for the series length
        n_timestamps = len(series)
        # If series is too short, use half of its length (with minimum of 2)
        actual_window_size = min(window_size, max(2, n_timestamps // 2))

        ssa = SingularSpectrumAnalysis(window_size=actual_window_size, groups=[[0]])
    ...
    def hurst_window(x):
        ...
        try:
            ...
            return 0.0  # Return NaN if computation fails
    ...
    def add_technical_indicators(self):
        ...
        if 'close_denoised_L1' in self.indicator_list:
            df['close_denoised_L1'] = df.groupby('Ticker')['Close'].apply(self.wavelet_denoise).reset_index(level=0,
                                                                                                            drop=True)
        ...
        if 'cci' in self.indicator_list:
            df['cci'] = df.groupby('Ticker')[['High', 'Low', 'Close', 'Volume']].apply(self.compute_cci).reset_index(
                level=0, drop=True)

        if 'parabolic_sar' in self.indicator_list:
            df['parabolic_sar'] = df.groupby('Ticker')[['High', 'Low', 'Close']].apply(
                self.compute_parabolic_sar).reset_index(level=0, drop=True)
