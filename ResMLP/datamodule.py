...
class CrossSectional():
    ...
    def build_frame_range(
        ...
        **kwargs,
        ...
        df = df.dropna(subset=["ret_1m"] + [f"{c}_rank" for c in cls.rank_cols])
...
class CrossSectionDS(Dataset):
    ...
    def __init__(self, frame: pd.DataFrame,
                 ...
                 target_col: str):
        self.X = torch.tensor(frame[feature_cols].values,
                              dtype=torch.float32)
        self.y = torch.tensor(frame[target_col].values,
...
try:
    import pytorch_lightning as pl
except ModuleNotFoundError:  # keep lightweight base install
    pl = None  # type: ignore
...
if pl is not None:
    ...
    class DataModule(pl.LightningDataModule):
        ...
        def prepare_data(self):  # type: ignore[override]
            ...
            self._data = CrossSectional.build_frame_range(
