...
class CharlieMungerAgent:
    ...
    def analyze(self):
        ...
        management_analysis = analyze_management_quality(financial_line_items, None)
        predictability_analysis = analyze_predictability(financial_line_items)
        valuation_analysis = calculate_munger_valuation(financial_line_items)
...
def analyze_moat_strength(financial_line_items: DataFrame):
    """
...
def analyze_management_quality(financial_line_items: DataFrame, insider_trades):
    """
    ...
    if debt_values is not None and equity_values is not None and len(debt_values) == len(equity_values):
...
def analyze_predictability(financial_line_items: DataFrame):
    """
    ...
    if financial_line_items.empty or len(financial_line_items) < 5:
        return {
            "score": 0,
    ...
    if revenues is not None and len(revenues) >= 5:
    ...
    if op_margins is not None and len(op_margins) >= 5:
    ...
    fcf_values = financial_line_items.free_cash_flow.values
    if fcf_values is not None and len(fcf_values) >= 5:
...
def calculate_munger_valuation(financial_line_items: DataFrame):
    """
